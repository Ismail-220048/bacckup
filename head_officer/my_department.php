 ...
 */
session_start();
if (!isset($_SESSION['user_id']) || !in_array($_SESSION['role'], ['senior_officer', 'district_admin', 'state_admin', 'national_admin'])) {
...
$officerDoc = $officersCol->findOne(['_id' => new MongoDB\BSON\ObjectId($officerId)]);
...
<body>
        ...
        <main class="main-content">
            ...
            <div class="page-header">
                <div class="header-left">
                    ...
                    <h1><i class="fa fa-list-alt"></i> Dept. Complaints (<?php echo htmlspecialchars($department); ?>)</h1>
                </div>
                <div class="user-info">
            ...
            <div class="card">
                    <div class="card-header">
                        <h3><i class="fa fa-list-alt"></i> Department Complaints (<?php echo count($complaintsList); ?>)</h3>
                    </div>
                                    ...
                                    <tr id="row-<?php echo $cId; ?>">
                                        ...
                                        <td>
                                            <div class="action-btns">
                                                <?php if (in_array($_SESSION['role'], ['senior_officer', 'district_admin', 'state_admin', 'national_admin'])): ?>
                                                    <button class="btn btn-info btn-sm" onclick="openUpdateModal('<?php echo $cId; ?>', '<?php echo htmlspecialchars($status); ?>', <?php echo htmlspecialchars(json_encode($c['officer_notes'] ?? '')); ?>)"><i class="fa fa-pencil-square-o"></i> Update/Assign</button>
                                                <?php endif; ?>
    ...
    <div class="modal-overlay" id="updateModal">
        <div class="modal" style="width: 100%; max-width: 600px; padding: 2.5rem; border-radius: var(--radius-lg);">
            <div class="modal-header" style="border-bottom: 2px solid var(--border); padding-bottom: 1rem; margin-bottom: 1.5rem;">
                <h3 style="font-size: 1.6rem; font-weight: 800; color: var(--gov-navy);"><i class="fa fa-pencil-square-o"></i> Manage Complaint</h3>
            ...
            <form id="updateForm">
                ...
                <!-- Senior Officer Assignment Dropdown -->
                <div class="form-group" style="margin-top:1rem;">
                    <label for="update-assignee">Assign to Officer</label>
                    <select id="update-assignee" name="assigned_officer_id" class="filter-select" style="width: 100%;">
                        <option value="">Keep Unchanged / Unassigned</option>
                        <?php 
                        foreach (iterator_to_array($deptOfficers) as $do) {
                             $selected = ($c['assigned_officer_id'] ?? '') === (string)$do['_id'] ? 'selected' : '';
                             echo '<option value="'.(string)$do['_id'].'" '.$selected.'>'.htmlspecialchars($do['name']).' ('.htmlspecialchars($do['role']).')</option>';
                        }
                        ?>
                    </select>
                </div>
    ...
    <script>
        ...
        document.getElementById('updateForm').addEventListener('submit', async function(e) {
            ...
            const assigneeId = document.getElementById('update-assignee').value;
            ...
            formData.append('assigned_officer_id', assigneeId);
