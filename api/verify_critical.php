...
if ($action === 'verify') {
    ...
    $notifCol->insertOne([
        'user_id'    => $officerId,
        'role'       => 'officer',
        'message'    => "🚨 URGENT: You have been assigned a VERIFIED CRITICAL complaint: " . $complaint['title'],
        'is_read'    => false,
    ...
    $notifCol->insertOne([
        ...
        'role'       => 'user',
        'message'    => "🚨 Your critical complaint has been VERIFIED and immediately assigned to " . $officerName,
        'is_read'    => false,
        'created_at' => date('Y-m-d H:i:s')
    ]);
    ...
    if (!empty($officerEmail)) {
        ...
        try {
            ...
            // Content
            $mail->isHTML(true);
            $mail->Subject = '🚨 URGENT: Critical CivicTrack Complaint Assigned to You';
            $mail->Body    = "<div style=\"font-family: Arial, sans-serif; color: #333; line-height: 1.6;\">" .
                             "<h2 style=\"color: #ef4444;\">🚨 Critical Response Required</h2>" .
                             ...
                             "<p style=\"margin: 25px 0;\"><a href=\"{$mapLink}\" style=\"display: inline-block; padding: 12px 20px; background-color: #2563eb; color: #ffffff; text-decoration: none; border-radius: 5px; font-weight: bold;\">🗺️ Start Live Tracking & Navigation on Google Maps</a></p>" .
                             ...
                             "<p><strong>CivicTrack Administration</strong></p>" .
                             "</div>";
            ...
            $mail->AltBody = "Hello " . $officerName . ",\n\n" .
                             ...
                             "🗺️ Track & Navigate Location: " . $mapLink . "\n\n" .
