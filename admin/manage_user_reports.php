...
if (!empty($userIds)) {
    ...
    if (!empty($validIds)) {
        $foundUsers = $usersCol->find(['_id' => ['$in' => array_map(fn($id) => new \MongoDB\BSON\ObjectId($id), $validIds)]]);
                        ...
                        <table>
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <?php $s = $r['status'] ?? 'Audit Requested'; ?>
                                        <span class="badge" style="background: <?php echo ($s === 'Audit Requested' ? 'var(--warning)' : ($s === 'Dismissed' ? 'var(--radius-sm)' : 'var(--danger)')); ?>; color: var(--gov-navy);">
